...
export const getStaticProps = async () => {
    const res = await fetch('http://127.0.0.1:8000/France')
...
const FranceHome = ({ london }) => {
    ...
    return (
        <div>
            ...
            <SideBar />
            <div id="london" className="grid mb-24 mx-4 gap-y-8 grid-cols-1 md:grid-cols-2 md:gap-x-16 lg:gap-x-8 xl:px-56">
                {london.map(items => (

                    <Link href={'/France/' + items.id} key={items.id}>
